#!/bin/sh
# run.sh -- build (if needed) and launch the FujiNet Game Lobby in the
# FujiNet-patched jzIntv, connected to a real fujinet-firmware instance
# over BoIP.
#
...
# last built.
if [ ! -f lobby.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer lobby.rom)" ]; then
    echo "Building lobby.rom..."
    make
fi
